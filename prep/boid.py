...
def get_boid(shader, fish, count, lower_limits, upper_limits):
    boid_shape = []
    positions = new_flock(count, lower_limits, upper_limits)
    for i in range(count):
        boid_shape.append(Node(transform=t.translate(positions[i][0], positions[i][1], positions[i][2]) @ t.scale(0.5)))
